const env = process.env.NODE_ENV || 'dev'

const config = () => {
    switch (env) {
        case 'dev':
            return {
                database: 'mongodb+srv://brino:123@cluster0-njrky.azure.mongodb.net/techConnective?retryWrites=true&w=majority',
                secret: '6f12ec104f3a9f9b4e2f7e669cc91314',
                expiresIn: '2d'
            }
        case 'prod':
            return {
                database: 'mongodb+srv://brino:123@cluster0-njrky.azure.mongodb.net/techConnective?retryWrites=true&w=majority',
                secret: '',
                expiresIn: 43200
            }
    }
}
console.log(`Iniciando a aplicação em ambiente: ${env.toUpperCase()}`);

module.exports = config();